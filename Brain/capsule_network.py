...
class LinearCaps(nn.Module):
	def __init__(self, routing_iters, gpu):
		...
		# W: [in_capsules, out_capsules, out_capsule_size, in_capsule_size] = [2041, 8, 16, 8]
	...
	def forward(self, x):
		# x: [batch_size, in_capsules=2041, in_capsule_size=8]
		batch_size = x.size(0)

		x = torch.stack([x] * self.out_capsules, dim=2)
		# x: [batch_size, in_capsules=2041, out_capsules=8, in_capsule_size=8]

		W = torch.cat([self.W.unsqueeze(0)] * batch_size, dim=0)
		# W: [batch_size, in_capsules=2041, out_capsules=8, out_capsule_size=16, in_capsule_size=8]

		# Transform inputs by weight matrix `W`.
		u_hat = torch.matmul(W, x.unsqueeze(4)) # matrix multiplication
		# u_hat: [batch_size, in_capsules=2041, out_capsules=8, out_capsule_size=16, 1]

		u_hat_detached = u_hat.detach()
		# u_hat_detached: [batch_size, in_capsules=2041, out_capsules=8, out_capsule_size=16, 1]
		...
		# b_ij: [in_capsules=2041, out_capsules=8, 1]
		...
		for iteration in range(self.routing_iters):
			...
			# c_ij: [batch_size, in_capsules=2041, out_capsules=8, 1, 1]
			...
			if iteration == self.routing_iters - 1:
				...
				# u_vj1: [in_capsules=2041, out_capsules=8, 1]
